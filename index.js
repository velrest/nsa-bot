    ...
    if (message.mentions.users.has(client.user.id)){
      if (message.content.match('cleanall')){
        if(permissionCheck(message.member, 'cleanall')){
          message.guild.channels.findAll('type', 'text').forEach(c => cleanChat(c, message.author))
        }
      }
    }
  },
...
client.on('voiceStateUpdate', (oldMember, newMember) => {
  if(oldMember.voiceChannelID != newMember.voiceChannelID){
    ...
    if(member.voiceChannelID in notificationsIn){
      getGuild().channels.get(notificationsIn[member.voiceChannelID]).send(
        joinMessage(message, {user: member.user.username, channel: member.voiceChannel.name})
      )
    }
    log(`\`${oldMember.user.username}\` changed from \`${(!oldMember.voiceChannelID?'emptiness':oldMember.voiceChannel.name)}\` to \`${(!newMember.voiceChannelID?'emptiness':newMember.voiceChannel.name)}\``);
  }
})
...
client.on('presenceUpdate', (oldMember, newMember) => {
  ...
  if(typeof newMember.presence.game !== 'undefined' && newMember.presence.game) {
    if(!newMember.presence.game.equals(oldMember.presence.game))
      log(`\`${newMember.user.username}\` is now playing \`${newMember.presence.game.name}\``);
  } else 
    log(`\`${newMember.user.username}\` stopped playing.`);
})
...
function permissionCheck(member, command){
  ...
  if(adminCheck(member.user) || memberHasRole(permissions.roles, member) || permissions.users.includes(member.id)){
    return true
  }
...
function cleanChat(channel, user){
  ...
  if (channel.id === config.clean.exlude && user.id !== config.admin){
    log(`\`${user.username}\` tried to clear \`${channel.name}\` without permission`)
  } else {
    channel.fetchMessages({limit : 100})
      .then(
        ms => {
          log('Cleaned in `'+channel.name+'` on behalf of `'+user.username+'`')
          ms.filter(
            m => {
              return (m.author.id === client.user.id || m.mentions.users.has(client.user.id)) ? true : false
            }
          ).array().forEach( m => m.delete() .catch(console.error))
        }
      )
      .catch(console.error)
  }
...
function log(msg) {
  ...
  getGuild().channels.get(config.log).send(msg).catch(console.error)
}
